...
from dashboard.models import omegadbconn

import sys
import pymssql

#주소값 가져오기 SQL 서버정보 (월드메모리)
server = '211.115.212.40:9025'
database = 'OMEGAPLUS_WM_TEST'
username = 'unilite'
password = 'UNILITE'
...
def connectsql(request):
    ...
    try:
        sql_conn = pymssql.connect(server, username, password, database, charset="UTF8")  # CP936

        cursor = sql_conn.cursor()
        # datas = []
        cursor.execute("SELECT TOP 10 CUSTOM_CODE, CUSTOM_NAME, VAT_RATE FROM BCM100T")
        ...
        # get specific field
        # cust = (row['CUSTOM_CODE'] for row in select)
        # print(select)
        ...
        print('Error')
        sys.exit()
    return render(request, 'dashboard/omega_eis.html', {'select':select})

    # result = omegadbconn.objects.all()
    # sql_conn = pymssql.connect(server, username, password, database, charset="UTF8")  # CP936
    # cursor = sql_conn.cursor()
    # sql = "SELECT TOP 10 CUSTOM_CODE, CUSTOM_NAME FROM BCM100T"
    # # cursor.execute("SELECT TOP 5 CUSTOM_CODE, CUSTOM_NAME FROM BCM100T")
    # result = omegadbconn.objects.raw(sql)
    # # result = cursor.fetchall()
    #
    # print(result)
    #
    # return render(request, 'dashboard/omega_eis.html', {'omegadbconn':result})
...
def sb_member(request):
    ...
    return render(request, 'dashboard/airplay_member.html', {'members': members})

def sms(request):
    if request.method == 'GET':
        return render(request, 'dashboard/sendsms.html')
        ...
        if send_msg == '':
            ...
            # result = '정상 발송되었습니다.'
        return render(request, 'dashboard/sendsms.html', {'error': return_msg})